...
# DEFAULT_EMPLOYER_ID = 1  # Replace with the ID of the default employer user
...
def reassign_jobs_on_soft_delete(sender, instance, **kwargs):
    ...
    if old_instance.is_deleted == False and instance.is_deleted == True:
        if instance.is_employer:  # Check if the user is an employer
            ...
            try:
                default_employer = getattr(instance, '_default_employer', None)
                # default_employer = kwargs.get('default_employer', None)
                print("default employer.:", default_employer)
                if not default_employer:
                    print("Error: No valid default employer found to reassign jobs.")
                    return
                ...
                print(f"All jobs of employer {instance.username} have been reassigned to the {default_employer.username} employer.")
...
def reassign_jobs_on_hard_delete(sender, instance, **kwargs):
    # Check if the employer is being marked as deleted
    try:
